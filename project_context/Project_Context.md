...
| --- | --- | --- | --- | --- |
| **Mayank Anand** | Team Lead & System Architect | `backend/app/api/`, `backend/app/contracts/` | Monorepo structure, Pydantic v2 contracts, API Gateway routes, Docker orchestration, and system integration. | FastAPI, SQLAlchemy, Docker Compose, Pydantic v2 |
| **Hariom** | Machine Learning Lead | `backend/app/ml/` | Slot-filling NER pipeline (`DeBERTa-v3`), contrastive bi-encoder fine-tuning (`BGE-M3`), and Qdrant vector index integration. | PyTorch, Hugging Face Transformers, Sentence-Transformers, Qdrant Client |
| **Harsh** | Algorithmic Rules & Graph Lead | `backend/app/matching/`, `backend/app/graph/` | Deterministic ASME B16.5 / ASTM validation engine, Neo4j graph ontology models, and Cypher spare-locator queries. | Python, Neo4j Driver, Cypher, NumPy/Polars, Pydantic v2 |
| **Smariddih** | Document Intelligence Lead | `backend/app/ingestion/` | Dual-path PDF parsing (PyMuPDF fast path), PaddleOCR image extraction for scanned MTCs, and text normalization. | Python, PaddleOCR, PyMuPDF (`fitz`), Pillow, FastAPI |
| **Shaurya** | Data Pipelines & Taxonomy Lead | `data/` | Synthetic CPSE catalog generation (IOCL, ONGC, BPCL), NER annotation generation, contrastive pair generation, and UNSPSC/GeM curation. | Python, Pandas, Polars, OpenPyXL, JSON Lines |
| **Ranvijay** | Frontend Developer | `frontend/` | Central procurement portal, batch drag-and-drop catalog deduplication studio, and Human-in-the-Loop (HITL) review triage queue. | Next.js 16 (App Router), React 19, TypeScript, Tailwind CSS, shadcn/ui |
