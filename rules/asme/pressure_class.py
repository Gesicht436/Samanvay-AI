...
def check_pressure_class(
    ...
    - Up-rating for flanged components = Tier 3 (bolt circle mismatch)
      unless it's a raw forging → Tier 2
    """
    ...
    q_idx = PRESSURE_CLASS_HIERARCHY.index(q_class) if q_class in PRESSURE_CLASS_HIERARCHY else -1
    c_idx = PRESSURE_CLASS_HIERARCHY.index(c_class) if c_class in PRESSURE_CLASS_HIERARCHY else -1

    if q_idx < 0 or c_idx < 0:
        return (DynamicCompatibilityTier.TIER_2_SUBSTITUTE, 0.82, None)

    if c_idx < q_idx:
        # Candidate has LOWER pressure rating — FATAL
        return (
            ...
            RuleViolation(
                ...
                explanation=(
                    f"FATAL PRESSURE DOWN-RATING: Candidate Class {c_class} is below "
                    f"required Class {q_class}. Installing a lower-rated component risks "
                    f"rupture under operating design pressure per ASME B16.5."
                ),
            ),
        )
    ...
    if normalized_type in INLINE_COMPONENT_TYPES:
        # In-line: dimensions preserved, safe over-rating = Tier 1
        score = max(0.95, 1.0 - 0.01 * (c_idx - q_idx))
    ...
    # Default for unclassified types: safe over-rating = Tier 1
    score = max(0.95, 1.0 - 0.01 * (c_idx - q_idx))
    return (DynamicCompatibilityTier.TIER_1_IDENTICAL, score, None)
