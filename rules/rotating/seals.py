...
def check_mechanical_seal_and_elastomer(
    ...
    import re
    raw_q = str(query_props.get("seal_plan", query_props.get("api_plan", ""))).upper().strip()
    raw_c = str(cand_props.get("seal_plan", cand_props.get("api_plan", ""))).upper().strip()
    ...
    is_toxic = query_props.get("toxic_service", False) or query_props.get("carcinogenic", False) or query_props.get("category_m", False)

    if q_plan and c_plan and q_plan != c_plan:
        is_q_dual = any(p in q_plan for p in DUAL_PRESSURIZED_PLANS)
        is_c_single = any(p in c_plan for p in SINGLE_UNPRESSURIZED_PLANS)
